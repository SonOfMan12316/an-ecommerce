...
/** https://localhost:8080/api/user/jockoo */
...
// update the password when there's valid session
                ...
                hashPassword(password)
                    .then((hashedPassword) => {
                        UserModel.updateOne({ username: data.username }, { password: hashedPassword })
                            ...
                            .catch(() => res.status(500).send({ error: 'Password Update Failed' }))
                    })
                    .catch((err) => res.status(500).send(err));
            })
            .catch((err) => res.status(400).send({ error: 'Username Not Found' }));
    } catch (err) {
        res.status(401).send(err);
    }
}
